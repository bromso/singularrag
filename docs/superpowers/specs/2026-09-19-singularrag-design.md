...
*Amended 2026-09-23 (knowledge design, `docs/superpowers/specs/2026-09-23-singularrag-knowledge-design.md`).* "Any LLM call" becomes "no query-time model call beyond one embedding; extraction and embeddings run through a local model runtime, opt-in API for extraction". The embeddings/vector-search non-goal above is superseded the same way; see §7's retired gate.
...
*Amended 2026-09-23 (knowledge design, `docs/superpowers/specs/2026-09-23-singularrag-knowledge-design.md`).* The "Local embeddings" gate above is retired: tier-two run 3 (`eval/README.md`) found the map already serves most of the gold for locate questions and the remaining gap is what the agent keeps, not what FTS5 misses, and the docs set (documents-design amendment above) showed a structural ranking problem — thin file rank and a thinly shared FTS bonus — not a synonym-coverage one, so a "locate below 0.6" trigger was never going to fire on the actual gap (prose paraphrase, entity and relation retrieval). Embeddings and entity/relation extraction ship unconditionally instead, over Ollama (knowledge spec §2). Index model gains six tables: `section_embeddings`, `entities`, `entity_vec`, `entity_mentions`, `relations`, `extract_queue` (knowledge spec §3). Schema version 4.

*Amended 2026-09-24 (journeys design, `docs/superpowers/specs/2026-09-24-singularrag-journeys-design.md`).* Two entity types join the closed list, `process` and `role`. Schema version 6 adds `steps` (one row per step per documenting section: process, ordinal, text, role entity or role text, section), `step_systems` (the systems a step touches) and `steps_cache` (the steps answer per section hash), and a `stage` column on `extract_queue` (`0` entities pending, `1` steps pending). A second prompt, `STEPS_PROMPT`, runs in the same tick and claim only for a section whose own extraction produced a `process` entity, and returns that process's ordered steps; the queue row goes only once both prompts are applied, so an outage between them resumes at the steps prompt. Steps resolve against the section's own entities by `norm_name` and follow the section on deletion like mentions and relations. "Documented in" (the step's section) and "implemented by" (the code the section mentions, then code, config keys and file stems whose name tokens contain a touched system's name as a run of whole tokens) are derived at read time and never stored.
...
*Amended 2026-09-23 (knowledge design, `docs/superpowers/specs/2026-09-23-singularrag-knowledge-design.md`).* A sixth tool, `entities {query, entities?, limit?}` (limit default 10, max 25, knowledge spec §4): after the freshness header, matched entities as `name (type): description`, then relations as `src → dst: description (path::heading, lines a-b)`, then `# N entities · M relations · K sections`; one `retrievals` row, `tool = "entities"`, no write. `repo_map` gains two optional arguments, `entities: string[]` and `themes: string[]`, matched against extracted entities and relations to seed files the same way a focus file does; `Reasons` gains `semantic`, `entities` and `theme` fields so a semantic, entity or theme match stays drawable. Six tools in all now; `init`, `hook` and `doctor` stay CLI subcommands, not tools.

*Amended 2026-09-24 (journeys design, `docs/superpowers/specs/2026-09-24-singularrag-journeys-design.md`).* `entities` renders a matched `process` as its ordered steps, one line each (`text — role: … — systems: … — path::heading`, plus `code: path::name (+N)` when implementing code is found), serves the linked code after the cited sections so the retrieval records it, and its footer counts steps (`# N entities · M relations · S steps · K sections`). In `repo_map`, a matched process seeds the code its touched systems implement, and `Reasons` gains `implements` (the system names), rendered "Implements Expensify". Still six tools; `INSTRUCTIONS` and the `entities` description say to ask `entities` for how a process works.
...
*Amended 2026-09-23 (knowledge design, `docs/superpowers/specs/2026-09-23-singularrag-knowledge-design.md`).* The Sigma map gains an entity overlay behind a `Files` / `Files + entities` view toggle: entity nodes coloured by type and sized by mention count, dashed edges to the sections that mention them, solid edges between related entities; the detail panel for an entity lists its mentions and relations, and for a section lists the entities it mentions and the relations it states (knowledge spec §5). The status line gains three segments alongside the roots, shown only when relevant: ` · entities: N pending`, ` · models: unavailable`, ` · embeddings: rebuilding`; the skipped sheet lists extraction failures as `extraction failed: <last_error>`.

*Amended 2026-09-24 (journeys design, `docs/superpowers/specs/2026-09-24-singularrag-journeys-design.md`).* A Journeys perspective joins the view toggle: a list of processes (name, step count, roles) with a labelled role filter, and the selected process as an ordered list whose steps show role, systems, a "Documented in" button that focuses the section row and up to three "Implemented by" buttons that focus the code rows. It reads `GET /api/processes` (behind the per-run token and origin check, never writing, capped at 200 processes and 30 steps each); selecting a step records no retrieval.
...
*Amended 2026-09-23 (documents design, `docs/superpowers/specs/2026-09-23-singularrag-documents-design.md`).* A second tier-one set, `eval/questions-docs.toml`, runs against this repository as the corpus: twelve questions in three categories (`locate-doc`, `doc-to-code`, `code-to-doc`), gold named `path::name` with a section's heading text as the name, exactly as `singularrag find` prints it. The hono set does not move. The gate for the documents design: hono recall at 4096 within 0.02 of its value before the branch, and the docs set at or above 0.6 at 4096. Results are in `eval/README.md` under "Tier one on documents". From 2026-09-24 the docs set is run against a pinned worktree of the documents-design merge base (`5ec066b`), not the live branch tree, because later work keeps adding documents of its own to this repository and the questions were written against one fixed state of it.

*Amended 2026-09-23 (knowledge design, `docs/superpowers/specs/2026-09-23-singularrag-knowledge-design.md`).* A third tier-one set, `eval/questions-prose.toml`, and a vault set outside the repo (`~/.singularrag/questions-vault.toml`, `[[root]] name = "vault"`) test prose retrieval: twelve questions each, four `paraphrase`/`entity`/`relation` (prose) or `paraphrase`/`entity`/`relation` (vault), gold are sections; `entity` and `relation` questions are also checked against the `entities` tool, cited when a gold section is among the served items. Gate: the prose and vault sets reach 0.6 with seeds on at 4096, seeds-off is reported alongside, and `entities` cites a gold section for at least 8 of the 12 entity and relation questions on each set (knowledge spec §6). The prose fixture committed for CI (`crates/singularrag-core/fixtures/prose/`) is only two documents and 112 tokens end to end, so seeds-off recall on it alone is 1.000 at every budget and it cannot discriminate; the recorded prose number instead runs the same two files inside a pinned worktree of `5ec066b` (`docs/superpowers/specs/2026-09-23-singularrag-knowledge-design.md` §6, amended 2026-09-24). Results are in `eval/README.md` under "Tier one on knowledge".

*Amended 2026-09-24 (journeys design, `docs/superpowers/specs/2026-09-24-singularrag-journeys-design.md`).* The prose set gains a fourth category, `process`: four handbook questions (ids `S1`–`S4`) whose answers are steps, gold are sections; sixteen questions in all. `singularrag eval --extraction <path>` loads a checked-in extraction (entities, relations and `steps` blocks) after indexing, and every `entity`, `relation` and `process` question is also put to the `entities` tool, reported in a `cited` column and one `cited N/M <category>` line per category. Gate: hono and docs within 0.02 of their recorded values at 4096, the prose mean not below its recorded seeds-off values, and a `process` question counted as cited only when the `entities` answer renders a step of a process that cites a gold section (amended by the final review, 2026-09-24: 2/4 with the checked-in steps, S3 and S4 naming no process, role or step system). Results are in `eval/README.md` under "Tier one on journeys".
