...
- **Resolution.** `process` must match a `process` entity of that section by `norm_name`; otherwise the answer is discarded and the section is marked done (a warn line, no retry). Each `role` resolves to a `role` entity of the section by `norm_name` when one exists, else `role_id` is null and `role_text` keeps the text (capped at 80). Each system resolves to a `system` entity of the section by `norm_name`; unresolvable systems are dropped. Empty step text drops the step; more than 30 steps are truncated to 30.
  ...
  *Amended 2026-09-24 (final review).* A role or system resolves by `norm_name` among the section's own entities whatever type the entity was first stored under (a `GitHub` first seen as an `organisation` still counts as the step's system), preferring one of the expected type; `process` still requires type `process`.
  *Amended 2026-09-24 (first real-model run).* The steps answer's process name wins. qwen2.5:7b-instruct typed `First day` and `incident channel` as the section's process where the steps prompt named `Onboarding` and `Incident process`, so the exact-name join discarded every answer. Now the name resolves to a `process` entity of the section by `norm_name`; else to an entity of that name anywhere in the index, retyped `process` and given a mention on this section; else to a new `process` entity mentioned by this section and embedded after the transaction. Only an empty process name is discarded. The entity prompt also asks for the process named as the section names it (usually its heading), which the model follows; the mis-typed entities it still emits stay as step-less processes.
...
- Gate: hono and docs stay within 0.02 of their recorded values at 4096; the prose set's mean does not fall below its recorded seeds-off values (0.250 / 0.653 / 0.722 at 1024 / 2048 / 4096); all four `process` questions cite gold through `entities` with the checked-in steps loaded. Seeds-on stays open until Ollama is installed; `singularrag eval --no-models` remains the one-command seeds-off run.
  *Amended 2026-09-24 (final review).* Name matching alone let a process question count as cited with every `steps` block removed, so the rule is stricter: a `process` question is cited only when a gold section is among the served items and the `entities` answer has a process hit with at least one rendered step whose section is that gold section. The `entities` tool also reaches a process through its steps: a matched role or step system adds the processes it acts in or is touched by (`via step`, at most five). Measured with the checked-in extraction at 1024, 2048 and 4096: **cited 2/4 process**. S3 misses because its query matches only the `Post-mortem` document entity, which is neither a process nor a role or system of any step, so no process and no step is rendered; S4 misses because its query matches only the `Single sign-on` concept, which no step names (the step system is `Okta`), so the Onboarding steps are never reached. The gate's "all four" is not met on this rule; questions and fixture are unchanged.
...
*Amended 2026-09-24 (first real-model run).* With qwen2.5:7b-instruct extracting the pinned corpus (243 sections), the gate is met: all four `process` questions cite their gold section through a rendered step at 1024, 2048 and 4096 with seeds on (2/4 with `--no-models`, as with the checked-in extraction: S3 and S4 reach their process only through the query embedding). Prose recall is 0.375 / 0.875 / 1.000 seeds on. Numbers and recipe in `eval/README.md`, "First real-model run".
