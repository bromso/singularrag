...
*Amended 2026-09-24 (first real-model run, `hop-v0`).* Four things the fake never showed, all found on the first extraction of the pinned corpus with qwen2.5:7b-instruct. (1) A section's stored text keeps its child sections and code fences as blank runs, so an H1 was a 95k-character "section" split into sixteen model calls of whitespace; the text sent to the model and to the embedder now has whitespace runs squeezed (`knowledge::squeeze_ws`; 500 parts became 223, one per section), while hashes and search keep the stored text. (2) A real 6,000-character part takes the 7B model about 35 s on a laptop, so tick calls use `TICK_TIMEOUT` (120 s); the query-time embed keeps 30 s. (3) A timeout, a malformed answer and a 5xx the server returns for one prompt (Ollama's "prediction aborted, token repeat limit reached" when a generation loops) are per-request failures that cost the section an attempt; only a transport failure or a missing model is an outage that stops the tick. Before this, one slow or looping section was retried as the oldest row forever. `CLAIM_STALE_MS` is 600 s to cover two prompts. (4) A tick that moved the queue re-ticks at once, with a job check before every immediate tick, instead of waiting the 30 s cadence after each section; the first tick after a refresh still waits one cadence so an agent's first call is not delayed.
...
*Amended 2026-09-24 (plan execution).* Two corpus rulings, both because a fixed question set drifted out from under the tree it was written for. First, the prose fixture alone is only 112 tokens end to end, so seeds-off recall is 1.000 at every budget from 256 to 4096 and it cannot discriminate paraphrase quality; the prose set is instead scored on the pinned merge-base tree (`git archive 5ec066b`) with the two fixture files copied to `docs/voyage.md` and `docs/handbook.md` (single root, bare paths, gold unchanged), which gives seeds-off recall 0.250 at 1024, 0.653 at 2048, 0.722 at 4096 — the CI test and `fixtures/prose/extraction.json` still use the two-file fixture alone, unaffected, since they only need the mechanism to run, not to discriminate. Second, `eval/questions-docs.toml` was written against this repository's tree at the documents-design merge base (`5ec066b`); nine further knowledge-v0 tasks (this spec, its plan, the prose fixture, and their own code and tests) added documents of their own to the live corpus, which dilutes the per-file FTS-hit share every document already competes for (§8's index-model amendment), and the docs set's recall on the live `HEAD` tree fell to 0.444 with no ranking change at fault. The docs set is scored against the pinned `5ec066b` tree from here on, same recipe (`git worktree add --detach`, index, run), not the live branch tree.

*Amended 2026-09-24 (first real-model run, `hop-v0`).* Gate met with the real model on the pinned corpus: prose recall 1.000 at 4096 seeds on (0.875 at 2048, 0.375 at 1024, where the semantic seed's candidates crowd the budget and seeds-off is 0.438); `entities` cites gold on 8 of 8 entity and relation questions; the docs set rises from 0.653 to 0.764 at 4096 seeds on. hono and the vault set are not yet run seeds on. See `eval/README.md`, "First real-model run".
