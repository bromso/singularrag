...
- **Seeds.** A query term that hits `sections_fts` seeds the section's file with `FTS_FILE_BOOST`, the same boost as a `symbols_fts` hit today. A `query_ident_match` on a section or document name counts like one on a symbol name. `Reasons` gains `body_hit: bool`, rendered in the UI as "Matches the text of the section". `RefBy`, pins, notes and the unreferenced fraction are unchanged; there is no per-kind budget share. *Amended 2026-09-23 after the gate run:* a file's FTS bonus is shared between its hits by strength, not evenly. A name hit weighs 1.0; a body hit weighs its FTS5 `bm25()` rank normalised so the strongest body hit in the file weighs 1.0; a symbol with both adds them; each hit's share is its weight over the file's total (`rank::hit_shares`). Files with only name hits split evenly as before. Measured: docs set 0.361 → 0.653 at 4096, hono unchanged at 0.771. Tried and reverted with numbers: excluding `key` symbols as reference targets (hono unchanged, docs 0.653 → 0.486).
...
- **Retrieval.** A query panel above the rail: a labelled text field, a budget select (1024, 2048, 4096), a submit button. Submit calls `POST /api/query {query, budget}`, which runs `repo_map` under the actor's session key `serve` with the label "UI" and records the retrieval, so it appears in the rail and the detail panel with served, cut and reasons like any agent call. The response is the new retrieval's id; the rail selects it. The live region announces "N served, M cut".
...
- The hono question set does not move. Gate for this spec: hono tier-one recall at 4096 within 0.02 of the value before this branch, and the docs set at or above 0.6 at 4096. *Outcome, 2026-09-23:* docs 0.653 (met); hono 0.771 against 0.792, a drop of 0.021, one gold symbol (T1 `compose`) just under the cut behind twelve hono document rows. Accepted by Jonas as the cost of documents sharing the budget with no per-kind share; the limit stands as written for the next spec. Tier two reruns later with the workflow condition set; the agent-facing text change in §5 is a second reason that run's delta is not attributable to one change.
